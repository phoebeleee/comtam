...
def cars_EDA(df):
    ...
    **연비(mpg)와 주요 변수들 간의 관계, 대륙별 특성, 연도별 변화** 등을 탐색합니다.
    ...
    cubicinches_mpg(df)
    # # 지역별 연비의 평균
    # df.groupby("continent")["mpg"].mean()
    # #df.groupby("continent")["mpg"].mean().reset_index()
...
def cars_predict(df):
    ...
    X = df[["cylinders", "cubicinches", "hp", "weightlbs", "time-to-60"]] #대괄호 2개
    y = df["mpg"]
...
#지금 현재의 파일이 실행될 때만 main() 함수를 실행하도록 하는 조건문. 여러 개 파일 작업할 때를 위해서
